...
QTextCursor tmpCursor = ui->textEdit->textCursor();
 ...
 //  tmpCursor.setPosition(QTextCursor::StartOfBlock);
  ...
  // tmpCursor.movePosition(QTextCursor::StartOfBlock, QTextCursor::MoveAnchor, 1);
   qDebug ()<< "------------>>>1";
...
//  tmpCursor.movePosition(QTextCursor::Up, QTextCursor::MoveAnchor, 4);
 ...
 else //当滚轮向下滑，靠近使用者
 {
  ...
  // tmpCursor.movePosition(QTextCursor::EndOfBlock, QTextCursor::MoveAnchor, 1);
   qDebug ()<< "------------>>>2";
  //  ui->textEdit->setTextCursor(tmpCursor);
  }
...
QString MainWindow::searchVersionFile(QString tPath, QString tName)
{
 ...
 QString dirpath = "E://Server//Repository//SVN//工作//软件平台源码//3、产品发布//1、郑州14号线//程序修改记录";
 //设置要遍历的目录
 QDir dir(dirpath);
 //设置文件过滤器
 QStringList nameFilters;
 //设置文件过滤格式
 nameFilters << "*.so"<<"*.txt"<<"*.doc";
...
void MainWindow::on_cmdButton_clicked()
{
 ...
 QString cmdOutStr=QString::fromLocal8Bit(m_process.readAllStandardOutput()); //程序输出信息
 qDebug()<<cmdOutStr;
